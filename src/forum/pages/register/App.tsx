...
import { RegisterForm } from "@/forum/components/auth/RegisterForm";

const Desktop = () => (
  <Container maxWidth="lg" sx={{ position: "relative", display: "grid", placeItems: "center", minHeight: "100vh" }}>
    <Paper
...
const Mobile = () => (
  <Paper sx={{ position: "relative", borderRadius: 0, width: 1 }} elevation={0}>
    <Stack>
      <Box sx={{ mb: 2, p: 2 }}>
        <Button
          sx={{ textWrap: "nowrap", width: "fit-content" }}
    ...
    <Box className="mode-dark" sx={{ position: "relative", bgcolor: "secondary.main", color: "text.primary" }}>
      <AuthIntro />
    </Box>
  </Paper>
);
...
function App() {
  ...
  return (
    <AppWrapper>
      ...
      <ScrollArea>{isMd ? <Desktop /> : <Mobile />}</ScrollArea>
    </AppWrapper>
  );
}
