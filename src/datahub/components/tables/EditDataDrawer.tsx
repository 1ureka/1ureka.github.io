...
import { z } from "zod";
import { useUpdateRow } from "@/datahub/hooks/update";
...
const EditDataDrawer = ({ open, onClose, params, rowData }: EditDataDrawerProps) => {
  ...
  const defaultValues = useMemo(() => {
    ...
    params.columns.forEach(column => {
      if (rowData && rowData[column.name] !== undefined && rowData[column.name] !== null) {
        values[column.name] = String(rowData[column.name]);
      } else {
        values[column.name] = "";
      }
    });
    return values;
  }, [params.columns, rowData]);

  // 取得主鍵欄位用於 WHERE 條件
  const primaryKeyData = useMemo(() => {
    if (!rowData) return {};
    
    const pkColumns = params.columns.filter(col => col.pk >= 1);
    ...
    // 如果沒有主鍵，使用所有原始資料作為條件
    return Object.keys(pkData).length > 0 ? pkData : rowData;
  }, [params.columns, rowData]);
  ...
  useEffect(() => {
    if (open && rowData) {
      ...
      params.columns.forEach(column => {
        if (rowData[column.name] !== undefined && rowData[column.name] !== null) {
          newValues[column.name] = String(rowData[column.name]);
        } else {
